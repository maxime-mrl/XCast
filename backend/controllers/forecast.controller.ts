...
const dataOrder = ["wind", "temp", "rain", "hum", "th"];
...
export function getCapabilities(_req: Request, res: Response) {
  ...
  fs.readdirSync(mapsPath).forEach((model) => {
    // get available times
    const availableTimes = fs.readdirSync(
      path.join(rootPath, "public", "map", model)
    );
    // get dataset (assume dataset is the same for each hours as it should) and sort it based on predefined order
    const datasets = fs
      .readdirSync(
        path.join(rootPath, "public", "map", model, availableTimes[0])
      )
      .sort((a, b) => dataOrder.indexOf(a) - dataOrder.indexOf(b));
    // get levels and file name for each dataset
    const parsedDataset: Record<
      string,
      {
        names: string[];
        levels: number[];
    ...
    datasets.forEach((dataset) => {
      ...
      const names: string[] = [];
      files.forEach((file) => {
        // parse unique levels and file names
        const splitted = file.split(/\-|\./);
        splitted.pop(); // remove tiff
        if (
          levels.find((level) => parseInt(splitted[1]) === level) === undefined
        )
          levels.push(parseInt(splitted[1])); // unique levels
        if (names.find((name) => splitted[0] === name) === undefined)
          names.push(splitted[0]); // unique file name
      });
...
export function getForecast(req: Request, res: Response) {
  // req.quey = { lat: number, lng: number, model:string, time:string }
  // parse req query checking entries
  if (
    typeof req.query.lat !== "string" ||
    isNaN(parseFloat(req.query.lat)) ||
  ...
  const selectedForecast = JSON.parse(
    fs
      .readFileSync(
        path.join(rootPath, "public", "data", "arome_fake_data.json")
      )
      .toString()
  );
