...
export const checkUser = async (user: Partial<User>) => {
  ...
  if (user.password) {
    if (user.password.length < minPasslength)
      throw {
        // done before the db validation because hash change length
  ...
  if (user.username && !/^[-_a-z0-9]+$/i.test(user.username))
    throw {
      // don't check length because arleady handled by model - this way is easier to change
...
export const checkAndParseSettings = (
  rawSettings: Partial<User["settings"]>
) => {
  const settings = { ...rawSettings };
  // const settings = JSON.parse(rawSettings);
  /* ------------------------ check forecast settings ------------------------ */
  if (settings.forecastSettings) {
    const { model, selected, level, maxHeight, position } =
  ...
  if (settings.units) {
    ...
    settings.units.forEach((value: { selected: string }) => {
      if (value && typeof value.selected !== "string")
        throw {
          message: "données invalides.",
          status: 400,
        };
