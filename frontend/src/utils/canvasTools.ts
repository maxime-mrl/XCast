...
export const generateYchart = ({
  ...
  return {
    ...
    displayed: [
      min,
      ...increments.filter((increment) => increment >= min && increment <= max),
    ],
    chartMargin: margin,
  } as chart;
};
...
export default class Canvas {
  ...
  constructor(parent: HTMLElement, xChart: chart, yChart: chart) {
    ...
    if (parent.querySelector("canvas")) {
      // if canvas arleady exist just link back to it
  ...
  getCoord = (x: number, y: number) => {
    ...
    return {
      ...
      y:
        this.size.height -
        (y - this.yChart.min) * yIncrement -
        this.yChart.chartMargin, // invert to start from bottom
      xIncrement,
      yIncrement,
    };
  };
  ...
  drawRectangle = (
    ...
    // get points in pixels
    const topLeft = this.getCoord(
      coords.left || this.xChart.min,
      coords.top || this.yChart.max
    );
    const bottomRight = this.getCoord(
      coords.right || this.xChart.max,
      coords.bottom || this.yChart.min
    );
  ...
  drawWindArrow = (
    ...
    this.ctx.roundRect(
      ...
      -thickness * 0.75,
      thickness * 0.75,
      length / 2 + thickness * 0.75,
      [size]
    );
    this.ctx.rotate((-90 * Math.PI) / 180); // right part
    this.ctx.roundRect(0, 0, thickness * 0.75, length / 2 + thickness * 0.75, [
      size,
    ]);
    // fill
    this.ctx.fillStyle = color;
