...
export default function Sounding() {
  ...
  return <div className="sounding" ref={containerRef}></div>;
}
...
function drawSounding(
  { drawRectangle, ctx, getCoord, drawLine }: Canvas,
  forecast: {
    level: number;
  ...
  // parse data
  const parsed = forecast.data.z
    .map((z, i) => {
      // cld_frac (cloud)
      drawRectangle(
        { top: forecast.data.z[i + 1], bottom: z, left: 30 },
        `rgba(80, 80, 80, ${forecast.data.cld_frac[i] / 100})`
      );
      return {
        level: z,
  ...
  // draw functions
  const drawContiniousLine = (
    dataKey: "temp" | "dew",
    defindedColor?: string
  ) => {
    // start the line
    ...
    for (let i = 1; i < parsed.length; i++) {
      ...
      const prev = {
        value: parsed[i - 1][dataKey],
        level: parsed[i - 1].level,
      };
      const coord = getCoord(parsed[i][dataKey], parsed[i].level);
      const instabilityLevel =
        (actual.value - prev.value) / ((actual.level - prev.level) / 100);
      const color = defindedColor
        ? defindedColor
